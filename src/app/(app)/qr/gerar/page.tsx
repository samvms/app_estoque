...
import { RefreshCcw, Hash, Tag, Layers } from 'lucide-react'

type Variante = {
  id: string
  sku: string
  nome_exibicao: string | null
  variacao: string // ✅ aqui
...
function labelVariante(v: Variante) {
  const desc = normalize(v.nome_exibicao) ? normalize(v.nome_exibicao) : normalize(v.variacao)
  return `${v.produto_nome_modelo} — ${desc} (${v.sku})`
}
...
export default function QrGerarPage() {
  ...
  async function carregarVariantes() {
    ...
    const { data, error } = await supabase.schema('lws').rpc('fn_listar_variantes_ativas')
    ...
    // ✅ normaliza compatibilidade: se a RPC antiga ainda retorna "cor", cai nela
    const raw = (data ?? []) as any[]
    const list: Variante[] = raw.map((r) => ({
      id: r.id,
      sku: r.sku ?? '',
      nome_exibicao: r.nome_exibicao ?? null,
      variacao: r.variacao ?? r.cor ?? '', // ✅ aqui
      produto_nome_modelo: r.produto_nome_modelo ?? r.produto_nome ?? '',
    }))
  ...
  async function gerar() {
    ...
    try {
      ...
      const { data, error } = await supabase.schema('lws').rpc('fn_criar_lote_qr', payload)
      ...
      >
        ...
        {bootLoading ? (
          ...
          <div className="space-y-4">
            ...
            {varianteSelecionada ? (
              <Card title="Prévia" subtitle="Confira antes de gerar." rightSlot={<Badge tone="ok">Pronto</Badge>}>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                  <StatCard title="Modelo">{varianteSelecionada.produto_nome_modelo}</StatCard>
                  <StatCard title="Variação">{varianteSelecionada.variacao}</StatCard>
            ...
            <div className="rounded-2xl border border-app-border bg-white p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="text-sm text-slate-600">Ao gerar, você será direcionado para o lote para imprimir o PDF.</div>
